...
function sndReq(lid) {
    var lsel = document.forms['fset_details'].leagues;
    var lid = lsel.options[lsel.selectedIndex].value;
    var msel = document.forms['fset_details'].matchtype;
    var mt  = msel.options[msel.selectedIndex].value;
    http.open('get', 'index.php?do=rpc&lgeid='+lid+'&type='+mt);
...
function sameday() {
    var sd = document.forms['fset_details'].startdate.value;
    if(!sd) return false;
    var ed = document.forms['fset_details'].enddate.value;
    if(!ed) {
      document.forms['fset_details'].enddate.value = sd;
    }
    return false;
}

function chng_date(p_or_m, dsel) {
  var fdd = "fixture_date"+dsel;
  var dd = document.getElementById(fdd);
  var nd = dd.options.length;
  var si = dd.selectedIndex; 

  if(p_or_m == "m" && si > 0) --si;
  if(p_or_m == "p" && si < nd-1) ++si;
  if(si != dd.selectedIndex) dd.options[si].selected = true;
  return true;
}
...
(function() {
    ...
    *
    * @namespace YAHOO.predict.calendar
    * @module calendar
    * @since 2.5.2
    ...
    */
    ...
    YAHOO.namespace("predict.calendar");
    YAHOO.predict.calendar.IntervalCalendar = IntervalCalendar;
})();
...
YAHOO.util.Event.onDOMReady(function() {
    ...
    var d = new Date();
    //var cal = new YAHOO.predict.calendar.IntervalCalendar("cal1Container", {pages:2, maxdate: d, mindate: "2009/09/01"});
    var cal = new YAHOO.predict.calendar.IntervalCalendar("cal1Container", {pages:2});
    cal.setMonth(d.getMonth());
    // If the star and end date input boxes have a value set then set the
    // interval in the calnedar
    if(inTxt && outTxt) {
      // Date is the form YYYY-MM-DD
      var symd = inTxt.value.split('-');
      var eymd = outTxt.value.split('-');
      var sdate = new Date(symd[0], symd[1]-1, symd[2]);
      var edate = new Date(eymd[0], eymd[1]-1, eymd[2]);
      cal.setInterval(sdate, edate);
    }
    ...
    cal.selectEvent.subscribe(function() {		    
	    ...
	    var ystr = inDate.getFullYear();
	    var mstr = inDate.getMonth()+1;
	    if(mstr.toString().length == 1) { mstr = "0" + mstr; }
	    var dstr = inDate.getDate();
	    if(dstr.toString().length == 1) { dstr = "0" + dstr; }
	    inTxt.value = ystr + "-" + mstr + "-" + dstr;
		...
		var ystr = outDate.getFullYear();
	        var mstr = outDate.getMonth()+1;
	        if(mstr.toString().length == 1) { mstr = "0" + mstr; }
	        var dstr = outDate.getDate();
	        if(dstr.toString().length == 1) { dstr = "0" + dstr; }
	        outTxt.value = ystr + "-" + mstr + "-" + dstr;
            }
        }
    }, cal, true);
